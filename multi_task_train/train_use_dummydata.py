...
def get_distributed_dataloader(args, dataset, num_workers=4, shuffle=True):
    ...
    if args.phase == 'pretrain':
        dataloader = DataLoader(
            dataset, batch_size=args.batch_size, collate_fn=dataset.collate_fn, num_workers=num_workers, pin_memory=True, sampler=sampler, drop_last=True
        )
    else:
...
def train():
    ...
    train_dataset = DummyDataset(
        image_w_h=(256, 256),
        src_dict_dim=src_tokenizer.vocab_size,
        tgt_dict_dim=tgt_tokenizer.vocab_size,
        src_max_length=256,
        tgt_max_length=256,
        length=int(1e6),
    )
