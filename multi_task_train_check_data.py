...
def train():
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        # print(len(pbar))
        # print(len(train_loader))  # 確認 visual_genome object_365 cc3m openimage
        for i, (src_images, tgt_images, src_texts, tgt_texts) in enumerate(tqdm(train_loader, disable=(world_rank != 0))):
            step = i + 1
            # prefix_text = f"epoch:{epoch} step:{step} rank:{world_rank} || "
            # print(prefix_text)
            src_texts = src_tokenizer.batch_decode(src_texts, skip_special_tokens=False)
            tgt_texts = tgt_tokenizer.batch_decode(tgt_texts, skip_special_tokens=False)
            image_dir = f"results/images/rank-{world_rank}"
            ...
            if i == 0 or i == len(train_loader) - 1:
                if epoch == args.start_epoch and i == 0:
                    text_f = open(f"results/rank-{world_rank}_src_tgt.txt", "w")
                else:
                    text_f = open(f"results/rank-{world_rank}_src_tgt.txt", "a")
                    text_f.write(f"epoch-{epoch} : step-{step}\n")
                for b in range(src_images.shape[0]):
                    ...
                    img.save(f"{image_dir}/epoch-{epoch}_step-{step}_batch-{b}.png")

                    text_f.write(f"batch-{b}: \n{src_texts[b]}\n{tgt_texts[b]}\n")
                text_f.write("\n\n\n\n")
                text_f.close()
