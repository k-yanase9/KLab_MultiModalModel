...
class VQModel(nn.Module):
    ...
    def encode(self, x):
        h = self.encoder(x)
        h = self.quant_conv(h)
        min_encoding_indices = self.quantize.encode(h)
        return h, min_encoding_indices

    def decode(self, h, min_encoding_indices):
        quant, emb_loss, info = self.quantize.decode(h, min_encoding_indices)
